...
def accounting():
    ...
    if send_ktd_form.send_ktd.data and send_ktd_form.validate():
        ...
        flash(res, 'send_ktd')
        if status:
            flash("https://polygonscan.com/tx/" + status, 'send_ktd')
    ...
    if send_kti_form.send_kti.data and send_kti_form.validate():
        ...
        flash(res, 'send_kti')
        if status:
            flash("https://polygonscan.com/tx/" + status, 'send_kti')
        return redirect(url_for('accounting'))
    if send_eth_form.send_eth.data and send_eth_form.validate():
        ...
        flash(res, 'send_eth')
        if status:
            flash("https://polygonscan.com/tx/" + status, 'send_eth')
        return redirect(url_for('accounting'))
    if set_budget_form.set_budget.data and set_budget_form.validate():
        ...
        if status:
            flash("https://polygonscan.com/tx/" + status, 'set_budget')
        return redirect(url_for('accounting'))
    ...
    if put_mark_form.put_mark.data and put_mark_form.validate():
        ...
        flash(res, 'put_mark')
        if status:
            flash("https://polygonscan.com/tx/" + status, 'put_mark')
        return redirect(url_for('accounting'))
    ...
    if exchange_ktd_to_eth_form.exchange_ktd_to_eth.data and exchange_ktd_to_eth_form.validate():
        ...
        if status:
            flash("https://polygonscan.com/tx/" + status, 'exchange_ktd_to_eth')
